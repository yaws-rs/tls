        ...
        loop {
            ...
            let (mut in_discard, rustls_state) = match self {
                Self::Server(ref mut s) => {
                    ...
                    (s_status.discard, TlsConnectionState::Server(ok_state))
                }
                Self::Client(ref mut c) => {
                    ...
                    (c_status.discard, TlsConnectionState::Client(ok_state))
                }
            };
            ...
            let pos = match rustls_state {
                ...
                TlsConnectionState::Server(RustlsConnectionState::TransmitTlsData(t)) => {
                ...
                TlsConnectionState::Client(RustlsConnectionState::TransmitTlsData(t)) => {
                    /* match t.may_encrypt_app_data() {
                        Some(w) => // send data
                    } */
                    t.done();
                ...
                TlsConnectionState::Client(RustlsConnectionState::WriteTraffic(mut wt)) => {
                    let (_r_in_len, r_out_len) = r.right_lens();
                    if r_out_len > 0 {
                ...
                TlsConnectionState::Client(c) => {
                    todo!("Client uninmplemented: {:?}", c)
                }
                TlsConnectionState::Server(s) => {
